...
self.addEventListener('fetch', (event: any) => {
  ...
  event.respondWith(
    caches.match(event.request)
      ...
      .catch((error: any): Promise<Response | undefined> => {
 ...
 */
function syncContactForm(): Promise<void> {
 ...
 */
function getAllPendingContactForms(): Promise<ContactForm[]> {
 ...
 */
function removePendingContactForm(id: number): Promise<void> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('fuelq-offline', 1);
