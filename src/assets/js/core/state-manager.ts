...
/**
 * State Manager
 * Centralized state management for application
 */
...
type SubscriberCallback = (value: any, key: string) => void;
...
class StateManager {
   ...
   */
  init(config: StateManagerConfig = {}): void {
   ...
   * @param {boolean} notify - Whether to notify subscribers
   */
  setMultiple(updates: StateManagerState, notify: boolean = true): void {
    // Save previous state to history
   ...
   */
  private saveToHistory(key: string, value: any): void {
    this.history.push({
      key,
      value,
      timestamp: Date.now()
   ...
   */
  getHistory(key: string): StateHistoryEntry[] {
