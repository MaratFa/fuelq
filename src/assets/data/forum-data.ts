...
async function createThread(threadData: ThreadData): Promise<Thread> {
  try {
    const response = await fetch(`${API_BASE_URL}/threads`, {
      method: "POST",
      headers: {
...
async function addComment(threadId: string, commentData: CommentData): Promise<Comment> {
  try {
...
// Export the threads array for direct access
export { threads };
